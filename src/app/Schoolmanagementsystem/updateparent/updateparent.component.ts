...
export class UpdateparentComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.schools.listparents<listparents[]>(school_id).subscribe(
      ...
      () => {
        this.schools.fetchsingleparents<any>(this.addacademic_id).subscribe(
          (data: any) => {
            this.addparent = data;
          },
          error => () => {
  ...
  addstudent() {
    ...
    this.addparent.updateDate = formatDate(new Date(), "MMM yyyy", "en-US");
    ...
    this.schools
      .updateparents<addparent>(this.addparent.id, this.addparent)
      .subscribe(
        (data: addparent) => {
          this.loader = false;
        },
        ...
        () => {
          this.loader = false;
          alert("Parent has been updated successfully");
