...
@Component({
  selector: "app-studentbulks",
  templateUrl: "./studentbulks.component.html",
  styleUrls: ["./studentbulks.component.scss"]
})
export class StudentbulksComponent {
  @ViewChild("fileImportInput", { static: false }) myInputVariable: ElementRef;

  parentdublicates: parentdublicates;
  academic: any;
  result: any;
  pcode: any;
  isholi: any;
  ...
  rfiddublicates: any;
  calenderdublicates: any;
  StudentPromotedublicates: any;
  ...
  constructor(
    ...
    this.parentdublicates = new parentdublicates();
    this.studentdublicates = new studentdublicates();
    this.rfiddublicates = new rfiddublicates();
    this.calenderdublicates = new calenderdublicates();
    this.StudentPromotedublicates = new StudentPromotedublicates();

    Amplify.configure(SchoolsService.cognitoconfig);
    StudentbulksComponent.location = this._location;
    ...
    this.route.params.subscribe(params => {
      this.uploadtype = params["id"]; // (+) converts string 'id' to a number
      if (this.uploadtype == "Student") this.title = "STUDENT";
      else if (this.uploadtype == "Parent") this.title = "PARENT";
      else if (this.uploadtype == "RFID") this.title = "RFID";
      else if (this.uploadtype == "Calendar") this.title = "CALENDAR";
      else if (this.uploadtype == "StudentPromote")
        this.title = "StudentPromote";
    });
  ...
  upload() {
    if (this.uploadtype == "Student") {
      this.schools
        .checkuploads2<studentdublicates[]>(
          ...
          () => {
            if (this.result.length == 0) {
              ...
              this.schools.bulkuploadlog<any>(dataip).subscribe(
                (datatoup: BulkUploadLog) => {
                  ...
                  this.bucket.upload(this.params, function (err, data) {
                    ...
                    StudentbulksComponent.location.back();
                  });
                },
                error => () => {
                  /* can provide allert for error on the api */
              ...
              for (let i = 0; i < this.result.length; i++) {
                let a =
                  "Duplication Error : " +
                  this.result[i].schoolStudentID +
                  " This Schoolstudent-id already Exist ";
                this.csvRecords.push(a);
              }
            }
          }
        );
    } else if (this.uploadtype == "Parent") {
      this.schools
        .checkuploads<parentdublicates[]>(this.parentdublicate, +this.schoolid)
        .subscribe(
    ...
    } else if (this.uploadtype == "RFID") {
      this.schools
        .checkdupliectionrfid<any[]>(this.rfiddublicates, +this.schoolid)
        .subscribe(
          (data: any[]) => {
            this.result = data;
          },
          ...
          () => {
            if (
              this.result.duplicateRfid.length == 0 &&
              this.result.existingActiveStudents.length == 0
            ) {
              let dataip = {
                schoolID: this.schoolid,
                type: "" + this.uploadtype,
              ...
              this.csvRecords = [];
              for (let i = 0; i < this.result.duplicateRfid.length; i++) {
                let a =
                  "Duplication Error : " +
                  this.result.duplicateRfid[i].rfiddata +
                  " This RFID is already exist ";
                this.csvRecords.push(a);
              }
              for (
                let i = 0;
                i < this.result.existingActiveStudents.length;
                i++
              ) {
                let a =
                  "Duplication Error : " +
                  this.result.existingActiveStudents[i].schoolStudentID +
                  " This School Student Id already assigned to another RFID ";
                this.csvRecords.push(a);
              }
            }
          }
        );
    } else if (this.uploadtype == "Calendar") {
      let dataip = {
        schoolID: this.schoolid,
        type: "" + this.uploadtype,
        updateID: this.activeloginuser.id,
  ...
  fileChangeListener($event: any): void {
    ...
    CSVFileValidator(this.file, config)
      .then(csvData => {
        ...
        if (csvData.inValidMessages.length > 0)
          alert("" + csvData.inValidMessages[0]);
      })
      .catch(err => {
        alert("check - dat err");
      });
    ...
    if (words.length > 1) {
      ...
      // this.errormsg = 'Please remove the white space in the file name';
    } else {
      this.params = {
  ...
  getDataRecordsArrayFromCSVFile(csvRecordsArray: any, headerLength: any) {
    if (csvRecordsArray[0] == "") {
      alert('The CSV file is empty');
      this.reset();
    } else if (csvRecordsArray.length == 1) {
      alert('The datas are is empty');
      this.reset();
    }
    else {
      if (this.title == "STUDENT") {
        ...
        for (let i = 1; i < csvRecordsArray.length; i++) {
          ...
          if (data.length == headerLength) {
            ...
            let SchoolStudentid = {
              schoolStudentID: csvRecord.SchoolStudentID
            };
            this.studentdublicates.push(SchoolStudentid);
            ...
            if (csvRecord.ShortName.length == 0) {
              this.dataArrerror.push(
                "In Row : " + [i] + " Column: Short Name is Empty"
              );
            }
            if (csvRecord.ShortName.length > 15) {
              this.dataArrerror.push(
                "In Row : " +
                [i] +
                " Column: Short Name is must be minimum 1 to 15 character"
              );
            }
            if (format.test(csvRecord.ShortName)) {
              this.dataArrerror.push(
                "In Row : " + [i] + " Column: Short Name Symbols  not allowed"
              );
            }
            ...
            if (csvRecord.GeoLocation.length < 10) {
              this.dataArrerror.push(
                "In Row : " +
                [i] +
                " Column: Geo Location must be minimum 10 to 250 character"
              );
            }
            csvRecord.Grade = data[5].trim();
            if (csvRecord.Grade.length == 0) {
              this.dataArrerror.push(
                "In Row : " + [i] + " Column: Grade is Empty"
              );
            }
      ...
      } else if (this.title == "StudentPromote") {
        this.studentdublicates = [];
        var dataArr = [];
        this.dataArrerror = [];
        for (let i = 1; i < csvRecordsArray.length; i++) {
        ...
        for (let i = 1; i < csvRecordsArray.length; i++) {
          ...
          if (data.length == headerLength) {
            ...
            var csvRecord: CSVRecord = new CSVRecord();
            csvRecord.FullName = data[0].trim();
            if (csvRecord.FullName.length == 0) {
              this.dataArrerror.push(
                "In Row : " + [i] + " Column: Full Name is Empty"
              );
            ...
            csvRecord.ShortName = data[1].trim();
            if (csvRecord.ShortName.length == 0) {
              this.dataArrerror.push(
                "In Row : " + [i] + " Column: Short Name is Empty"
              );
            ...
            this.parentdublicate.push(parentemail);
            if (csvRecord.Email.length == 0) {
              this.dataArrerror.push(
                "In Row :  " + [i] + " Column: Email is Empty"
              );
            }
            if (!EMAIL_REGEXP.test(csvRecord.Email)) {
              this.dataArrerror.push(
                "In Row :  " + [i] + " Column: Enter Valid Email id in Email"
              );
            }
            csvRecord.AltEmailId = data[3].trim();
            csvRecord.pcountrycode = data[4].trim();
            if (csvRecord.pcountrycode.length == 0) {
              this.dataArrerror.push(
                "In Row : " + [i] + " Column: Primary  country code is Empty"
              );
            ...
            csvRecord.PrimaryMobile = data[5].trim();
            ...
            csvRecord.altcountrycode = data[6].trim();
        ...
        for (let i = 1; i < csvRecordsArray.length; i++) {
          ...
          if (data.length == headerLength) {
            ...
            let EMAIL_REGEXP = /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/;
            var csvRecord: CSVRecord = new CSVRecord();
            let UpdateDate = formatDate(new Date(), "dd MMM yyyy", "en-US");
            let updateID = this.activeloginuser.id;
            let updateName = this.activeloginuser.first_name;
            let schoolID = this.getFromLocal("selected_school_id");

            let isActive = true;
            let a = {
              schoolID: schoolID,
              SchoolStudentID: data[0].trim(),
              isActive: isActive,
              rfiddata: data[1].trim(),
              updateID: updateID,
              updateName: updateName,
              UpdateDate: UpdateDate,
              uniquenumber: data[2].trim(),
              type: data[3].trim(),
              freequency: data[4].trim(),
              capacity: data[5].trim(),
              status: data[6].trim(),
              driver_id: data[7].trim()
            };
            this.rfiddublicates.push(a);
            ...
            if (csvRecord.SchoolID.length > 15) {
              this.dataArrerror.push(
                "In Row : " +
                [i] +
                " Column: School Student ID  is must be minimum 1 to 15 character"
            ...
            if (format.test(csvRecord.SchoolID)) {
              this.dataArrerror.push(
                "In Row : " +
                [i] +
                " Column: School Student ID Symbols  not allowed"
            ...
            csvRecord.RFID_Tag = data[1].trim();
            ...
            if (csvRecord.RFID_Tag.length > 15) {
              this.dataArrerror.push(
                "In Row : " +
                [i] +
                " Column: RFID Tag is must be minimum 3 to 15 character"
              );
            }
            ...
            csvRecord.unique_number = data[2].trim();
            ...
            if (csvRecord.unique_number.length < 3) {
              this.dataArrerror.push(
                "In Row : " +
                [i] +
                " Column: unique is must be Max 3 character"
              );
            }
            ...
            csvRecord.freequency = data[4].trim();
            ...
            if (csvRecord.freequency.length < 3) {
              this.dataArrerror.push(
                "In Row : " +
                [i] +
                " Column: Freequency is must be Max 3 character"
              );
            }
            ...
            csvRecord.capacity = data[5].trim();
            ...
            csvRecord.driver_id = data[7].trim();
            if (csvRecord.driver_id.length == 0) {
              this.dataArrerror.push(
                "In Row : " + [i] + " Column: driver_id  is Empty"
              );
            }
            ...
            if (csvRecord.driver_id.length > 15) {
              this.dataArrerror.push(
                "In Row : " +
                [i] +
                " Column: driver_id is must be minimum 1 to 15 character"
              );
            }
            if (format.test(csvRecord.driver_id)) {
              this.dataArrerror.push(
                "In Row : " + [i] + " Column: driver_id Symbols  not allowed"
              );
            }
            dataArr.push(csvRecord);
          }
        ...
        for (let i = 1; i < csvRecordsArray.length; i++) {
          ...
          if (data.length == headerLength) {
            ...
            if (csvRecord.SchoolID.length == 0) {
              this.dataArrerror.push(
                "In Row : " + [i] + " Column: Title  is Empty"
              );
            }
            if (csvRecord.SchoolID.length > 15) {
              this.dataArrerror.push(
                "In Row : " +
                [i] +
                " Column: Title  is must be minimum 1 to 15 character"
              );
            }
            if (format.test(csvRecord.SchoolID)) {
              this.dataArrerror.push(
                "In Row : " + [i] + " Column: Title Symbols  not allowed"
              );
            }
            csvRecord.ApplicableClassStart = data[1].trim();
            if (csvRecord.ApplicableClassStart.length == 0) {
              this.dataArrerror.push(
                "In Row : " + [i] + " Column: Applicable Class Start  is Empty"
              );
            }
            for (let j = 0; j < this.listgrades.length; j++) {
              if (csvRecord.ApplicableClassStart == this.listgrades[j].title) {
                this.startc = this.listgrades[j].position;
                this.grade = 1;
              }
            }
            ...
            if (this.grade !== 1) {
              this.dataArrerror.push(
                "In Row :  " +
                [i] +
                " Column: Applicable Class Start must be in the Grade list"
              );
            }
            csvRecord.ApplicableClassEnd = data[2].trim();
            if (csvRecord.ApplicableClassEnd.length == 0) {
              this.dataArrerror.push(
                "In Row : " + [i] + " Column: Applicable Class Start  is Empty"
              );
            }
            for (let j = 0; j < this.listgrades.length; j++) {
              if (csvRecord.ApplicableClassEnd == this.listgrades[j].title) {
                this.endc = this.listgrades[j].position;
                this.grade1 = 1;
              }
            }
            if (this.grade1 !== 1) {
              this.dataArrerror.push(
                "In Row :  " +
                [i] +
                " Column: Applicable Class End must be in the Grade list"
              );
            }
            if (this.startc > this.endc) {
              this.dataArrerror.push(
                "In Row :  " +
                [i] +
                " Alert: The position of the  start class is lesser than the end class must be in the Grade list"
              );
            }
            csvRecord.ApplicableStartDate = data[3].trim();
            this.sdate = csvRecord.ApplicableStartDate;
            if (csvRecord.ApplicableStartDate.length == 0) {
              this.dataArrerror.push(
                "In Row : " + [i] + " Column: Applicable Start Date  is Empty"
              );
            }
            if (!pattern.test(csvRecord.ApplicableStartDate)) {
              this.dataArrerror.push(
                "In Row : " +
                [i] +
                " Column: Applicable Start Date format should be yyyy-mm-dd"
              );
            }
            csvRecord.ApplicableEndDate = data[4].trim();
            this.edate = csvRecord.ApplicableEndDate;
            if (csvRecord.ApplicableEndDate.length == 0) {
              this.dataArrerror.push(
                "In Row : " + [i] + " Column: Applicable End Date  is Empty"
              );
            }
            if (!pattern.test(csvRecord.ApplicableEndDate)) {
              this.dataArrerror.push(
                "In Row : " +
                [i] +
                " Column: Applicable End Date format should be yyyy-mm-dd"
              );
            }
            if (this.sdate > this.edate) {
              this.dataArrerror.push(
                "In Row : " +
                [i] +
                " Alert: Start date and end date may or may not be of same date but end date should not be lesser than start date "
              );
            }
            csvRecord.ApplicableStartTime = data[5].trim();
            this.startt = csvRecord.ApplicableStartTime;
            if (csvRecord.ApplicableStartTime.length == 0) {
              this.dataArrerror.push(
                "In Row : " + [i] + " Column: Applicable Start Time  is Empty"
              );
            }
            if (!time.test(csvRecord.ApplicableStartTime)) {
              this.dataArrerror.push(
                "In Row : " +
                [i] +
                " Column: Applicable Start Time  should be HH:MM:SS 24hrs Format"
              );
            }
            csvRecord.ApplicableEndTime = data[6].trim();
            this.endt = csvRecord.ApplicableEndTime;
            if (csvRecord.ApplicableEndTime.length == 0) {
              this.dataArrerror.push(
                "In Row : " + [i] + " Column: Applicable End Time is Empty"
              );
            }
            if (!time.test(csvRecord.ApplicableEndTime)) {
              this.dataArrerror.push(
                "In Row : " +
                [i] +
                " Column: Applicable End Time  should be HH:MM:SS 24hrs Format"
              );
            }
            if (this.startt > this.endt) {
              this.dataArrerror.push(
                "In Row : " +
                [i] +
                " Alert:  The Start time should be lesser than End time"
              );
            }
            csvRecord.isHoliday = data[7].trim();
            if (csvRecord.isHoliday.length == 0) {
              this.dataArrerror.push(
                "In Row : " + [i] + " Column: is Holiday is Empty"
              );
            }
            if (csvRecord.isHoliday! == "0" || csvRecord.isHoliday! == "1") {
              this.isholi = 1;
            }
            if (this.isholi == 0) {
              this.dataArrerror.push(
                "In Row : " +
                [i] +
                " Column: Holiday is only 1 or 0 1=true,0 =false"
              );
            }
            dataArr.push(csvRecord);
          }
...
export class rfiddublicates {
  schoolstudentid: string;
}

export class calenderdublicates {
  schoolstudentid: string;
}

export class StudentPromotedublicates {
  schoolstudentid: string;
}
