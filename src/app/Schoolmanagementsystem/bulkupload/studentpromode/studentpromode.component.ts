...
@Component({
  selector: "app-studentpromode",
  templateUrl: "./studentpromode.component.html",
  styleUrls: ["./studentpromode.component.scss"]
})
export class StudentpromodeComponent {
      ...
      for (let i = 1; i < csvRecordsArray.length; i++) {
        ...
        if (data.length == headerLength) {
          ...
          if (csvRecord.AltEmailId.length == 0) {
            this.dataArrerror.push(
              "In Row :  " + [i] + " Column: Alternate Email id is Empty"
            );
          }
          if (!EMAIL_REGEXP.test(csvRecord.AltEmailId)) {
            this.dataArrerror.push(
              "In Row :  " +
              [i] +
              " Column: Enter Valid Email id in Alternate Email id"
            );
          }
          ...
          if (
            csvRecord.AltMobile.length < 6 ||
            csvRecord.AltMobile.length > 15
          ) {
            this.dataArrerror.push(
              "In Row : " +
              [i] +
