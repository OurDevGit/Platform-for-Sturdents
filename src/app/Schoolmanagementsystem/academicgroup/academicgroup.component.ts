...
@Component({
  selector: "app-academicgroup",
  templateUrl: "./academicgroup.component.html",
  styleUrls: ["./academicgroup.component.scss"]
})
export class AcademicgroupComponent implements OnInit {
  deletecheck: boolean;
  showdisplay: boolean;
  accesslist: any;
  users: any[];
  ...
  exportasExcell() {
    this.exportExcell.exportAsExcelFile(this.datas_exp, "Academic");
  }
  ...
  ngOnInit() {
    ...
    for (let i = 0; i < this.accesslist.length; i++) {
      if (this.accesslist[i].code == "SAVAG") {
  ...
  update(event, item) {
    ...
    for (let i = 0; i < this.accesslist.length; i++) {
      if (this.accesslist[i].code == "SAEAG") {
        this.deletecheck = true;
      }
    }
    if (this.deletecheck == true) {
      this.router.navigate(["School/updateacademic/" + item.id]);
    } else {
  ...
  deletelist() {
    this.deletecheck = false;
    for (let i = 0; i < this.accesslist.length; i++) {
      if (this.accesslist[i].code == "SADAG") {
        this.deletecheck = true;
      }
    }
    if (this.deletecheck == true) {
      for (let i = 0; i < this.checkedList.length; i++) {
        this.schools
          .deleteacadamics<AcadamicGroup[]>(this.checkedList[i])
          .subscribe(
            (data: any) => {},
            error => () => {
              /* can provide allert for error on the api */
  ...
  deleteacadamic(event, item) {
    ...
    this.schools.deleteacadamics<AcadamicGroup[]>(item.id).subscribe(
      (data: any) => {},
      error => () => {
        /* can provide allert for error on the api */
      },
