...
@Component({
  selector: 'app-listwaypoints',
  templateUrl: './listwaypoints.component.html',
  styleUrls: ['./listwaypoints.component.scss']
})
export class ListwaypointsComponent implements OnInit {
  filter: any;
  deletecheck: boolean;
  showdisplay: boolean;
  accesslist: any;
  ...
  ngOnInit() {
    ...
    this.schools.waypointlists<any[]>().subscribe(
      (data: any) => {
        ...
        data.forEach(datas => {
          let a = {
            wayPointId: datas.WayPointId,
            wayPointName: datas.WayPointName,
            wayPointSeq: datas.WayPointSeq,
            geoPointText: datas.GeoPointText,
  ...
  update(event, item) {
    ...
    if (this.deletecheck == true) {
      this.router.navigate(["School/updatewaypoints/" + item.Id]);
    } else {
  ...
  view(event, item) {
    this.router.navigate(["School/viewwaypoints/" + item.Id]);
  }
