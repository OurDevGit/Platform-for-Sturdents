...
@Component({
  selector: "app-noncynosureuser",
  templateUrl: "./noncynosureuser.component.html",
  styleUrls: ["./noncynosureuser.component.scss"]
})
export class NoncynosureuserComponent implements OnInit {
  filter: any;
  deletecheck: boolean;
  showdisplay: boolean;
  showEditDisplay: boolean;
  showDeleteDisplay: boolean;
  accesslist: any;
  datas: UsersList[];
  ...
  exportasExcell() {
    this.exportExcell.exportAsExcelFile(this.datas_exp, "achoolusers");
  }
  ...
  newsync() {
    ...
    if (this.noncynosureuser.length > endval) {
    } else {
      endval = this.noncynosureuser.length;
    }
    this.pagedata = [];
    for (let index = startval; index < endval; index++) {
      this.pagedata.push(this.noncynosureuser[index]);
    }
  }
